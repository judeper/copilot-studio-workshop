...
#### Overview
In this lab, you will prepare the shared Woodgrove Bank workshop scenario used throughout Day 1. You will verify participant prerequisites, activate Copilot Studio access, select the correct Power Platform environment, and create the SharePoint site and lists that later labs use for grounding, Product Applications, and automated ticket handling.
...
2. Select the **Security groups** tab, select **Add a security group**, and create a group named `WoodgroveBankAgents`.
3. Open the new `WoodgroveBankAgents` group, select **Members**, select **View all and manage members**, and add the participant accounts.
4. Open `https://admin.powerplatform.com`, select **Manage**, then select **Tenant settings**.
5. Scroll to **Copilot Studio authors**, select the **Edit** pencil, choose the `WoodgroveBankAgents` security group, and select **Save**.
...
#### Step 5 — Create the Woodgrove Bank SharePoint site
...
5. In the **Site name** field, enter `Woodgrove Bank`.
6. In the **Site description** field, enter `Copilot Studio workshop data source`.
7. In the **Site address** field, enter `WoodgroveBank` or the nearest available variation.
...
> **Tip:** If your SharePoint template did not provision a **Customer Accounts** list or the columns differ from the steps above, select **New** > **List** > **Blank list**, name it `Customer Accounts`, and add the columns manually: Title (default), Manufacturer (Text), Model (Text), Asset Type (Text), Color (Text), Serial Number (Text), Purchase Date (Date), Purchase Price (Currency), Order # (Text), Status (Choice with values Available, Requested, Retired), and Image (Hyperlink).

#### Step 7 — Prepare the Service Requests list for later labs
1. Return to **Site contents** and open the **Service Requests** list.
...
5. Keep the **Service Requests** list available because Lab 10 uses it for event triggers.
...
4. Prepare a short slide or whiteboard note with the standard values `Woodgrove Bank`, `Woodgrove Customer Service Agent`, and `WoodgroveBankAgents` so naming stays consistent.
