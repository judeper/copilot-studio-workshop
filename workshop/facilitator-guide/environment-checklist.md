...
- `SharePoint.PnPLoginMode` defaults to `OSLogin` (Windows native sign-in via WAM) with automatic fallback to `DeviceLogin`. The bootstrap wizard ensures the Entra app's SharePoint `oauth2PermissionGrant` (`AllSites.FullControl`) is created — without this, PnP tenant admin operations fail even for Global Admins.
