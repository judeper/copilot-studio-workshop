...
2. The Contoso IT SharePoint site exists and the Devices list contains sample device records.
3. The Tickets list exists with at least one sample ticket and the site URL is captured for later labs.
...
| SharePoint site creation stalls or finishes partially. | Site provisioning is still running or the wrong site template was chosen. | Keep the page open until provisioning completes, then confirm the IT help desk template and retry only if needed. |
...
- Facilitator is ready to anchor discussion in the Contoso IT scenario.
...
| Participant confuses knowledge grounding with actions. | The concepts were explained abstractly without concrete examples. | Use the device lookup as knowledge and SharePoint item creation as the action contrast. |
...
### Success Criteria
1. A declarative agent named for the Contoso support scenario exists with the lab-provided description and instructions saved.
2. The agent has an `IT Expert` prompt tool configured and the prompt returns structured IT guidance in the test experience.
3. The published or previewed agent responds to an IT support prompt with job-related guidance rather than a generic answer.
...
### Success Criteria
1. The participant creates or opens the `Contoso Helpdesk Agent` solution and exports a solution package.
...
- The Contoso IT SharePoint site URL is available.
...
### Success Criteria
1. The `Contoso Helpdesk Agent` includes the intended web, SharePoint, and file knowledge sources, and web search is turned off.
2. A warranty-style support question returns a grounded answer with a visible citation.
3. A device-availability or quick-reference question is answered from the configured workshop knowledge instead of hallucinated content.
...
| The agent answers without grounding from the Devices list. | SharePoint indexing is incomplete or the filter excludes the needed content. | Wait for indexing, loosen the filter, and ask a more specific test question. |
...
### Pre-conditions
- Lab 06 is complete.
- The Devices list contains available devices to query.
- Participant can create topics and connector-based actions.

### Success Criteria
1. The `Available devices` topic exists and includes a SharePoint action that retrieves available device records.
2. Asking for available laptops or devices returns a structured list from SharePoint.
3. The topic asks the user whether they want to request one of the returned devices.

### Common Failure Modes
| Symptom | Root Cause | Resolution |
| --- | --- | --- |
| The topic does not trigger for device questions. | The description is too vague or orchestration is off. | Turn orchestration on, add concrete device keywords, and retest in a new session. |
...
| The topic returns blank or static output instead of the device list. | Variable storage or formula usage is incorrect. | Start with a simpler output, confirm the action data is present, then reintroduce dynamic formatting. |

### Facilitator Escalation Trigger
Escalate if the topic cannot retrieve any live SharePoint device data after connector and list validation.

## Lab 08 — Adaptive Cards
### Pre-conditions
- Lab 07 is complete and returns available devices.
- The adaptive card designer is available.
- Participant can edit a topic that uses card inputs.

### Success Criteria
1. The `Request device` topic contains an adaptive card node with device, manager email, and comments inputs.
2. The card appears after the user opts to request a device and the device choices are populated correctly.
3. Submitting the card captures the selected device and text inputs for downstream automation.
...
| The card shows empty or incorrect device choices. | The prior topic did not populate the device variable or field names do not match. | Run the availability topic in the same session and confirm the field references before retesting. |
...
- The Devices list contains at least one available device.
- SharePoint and Outlook connectors are allowed in the environment.
- The Device Requests list exists or can be created during the lab.

### Success Criteria
1. The Device Requests list exists with the fields needed to track a request.
2. The device-request flow is published and includes the expected retrieval, create, update, notification, and respond-to-agent steps.
3. A full test run creates a pending request record, updates the original device status, sends the notification, and returns a confirmation in chat.

### Common Failure Modes
| Symptom | Root Cause | Resolution |
| --- | --- | --- |
| The flow cannot update the Devices item. | Required SharePoint fields were not mapped in the update step. | Map all required columns from the original item, not just the status field. |
...
| The flow fails when trying to create the request item. | The Device Requests list or expected columns do not exist yet. | Create or correct the list first, refresh the flow designer, and retest from the start. |
...
### Pre-conditions
- Lab 06 is complete.
- The Tickets list exists.
- SharePoint and Outlook connectors are allowed.
- Agent orchestration is enabled.

### Success Criteria
1. A SharePoint-based trigger is configured to react to new ticket items.
2. The agent has an email action configured for ticket acknowledgment.
3. Creating a new ticket causes an acknowledgment email to be sent without a user starting a chat session.

### Common Failure Modes
| Symptom | Root Cause | Resolution |
| --- | --- | --- |
| Creating a ticket does not trigger any activity. | The trigger is disabled, unpublished, or was created after the test item. | Enable and publish the trigger, then create a brand-new test ticket. |
...
| The email is sent with blank ticket details. | The trigger payload mapping uses the wrong SharePoint fields. | Rebuild the field mapping using dynamic content from the current list schema. |
...
| Symptom | Root Cause | Resolution |
| --- | --- | --- |
| Participant assumes Microsoft 365 Copilot licensing covers all workshop automations. | They do not connect actions and autonomous workflows to Copilot Studio consumption. | Re-anchor the explanation in the device-request and trigger examples. |
...
- The Operative solution package and CSV files are downloaded locally.
- Dataverse, Teams, and Power Automate access is available in the tenant.

### Success Criteria
1. The Operative solution imports successfully and the Hiring Hub model-driven app opens.
2. Job Roles and Evaluation Criteria data are visible after the CSV imports.
3. The Hiring Agent exists with the expected baseline settings, including orchestration, file uploads, moderation, and user reactions.
...
| CSV import finishes with bad mappings. | Lookup fields or headers were mapped incorrectly. | Re-import with explicit field mapping and verify the Job Role lookup target. |
...
### Facilitator Escalation Trigger
Escalate if Dataverse or solution import issues block the Hiring Agent foundation for Day 2.

## Lab 14 — Agent Instructions
### Pre-conditions
- Lab 13 is complete.
- The Hiring Agent is open for editing.
- Participant can run immediate tests in the Copilot Studio test pane.

### Success Criteria
1. The Hiring Agent contains the balanced instruction set from the lab, including next-action guidance and scoped behavior.
2. An in-scope hiring prompt gets a hiring-focused answer and an off-topic prompt is redirected appropriately.
3. An ambiguous hiring prompt causes the agent to ask a clarifying question instead of guessing.
...
- The Hiring Agent is ready to orchestrate to child or connected agents.

### Success Criteria
1. The Hiring Agent is configured to let other agents connect and includes delegation guidance in its instructions.
2. Application Intake Agent and Interview Prep Agent are present with scoped responsibilities.
...
- The Resumes Dataverse table exists.
- A sample PDF resume is available.
- SharePoint and Teams connectors are permitted.

### Success Criteria
1. The Incoming Resumes library exists and the upload trigger is configured against it.
2. The automation flow is published with PDF filtering, Dataverse row creation, file upload, and Teams notification steps.
3. Uploading a new PDF creates the resume record, stores the file, and posts the notification.
...
### Pre-conditions
- Labs 13 and 14 are complete.
- The Hiring Agent can switch models in the environment.
...
| The participant cannot explain the final choice. | They used the picker without filling in the scorecard. | Re-run one comparison and frame the decision in terms of quality, speed, and recruiter trust. |
...
### Pre-conditions
- Labs 13 and 14 are complete.
- The Hiring Agent is open and editable.
- Participant has a short candidate summary for prompt testing.
...
### Facilitator Escalation Trigger
Escalate if the environment cannot enforce the required moderation controls or produces unsafe hiring outputs after guided remediation.

## Lab 19 — Multimodal Prompts
### Pre-conditions
- Lab 13 is complete.
- Participant can create prompt tools with document or image input.
- One text-based PDF resume and one image-based resume are available.
- A GA multimodal-capable model is available, with GPT-5 Chat preferred when supported.

### Success Criteria
1. The `Resume Vision Analysis` prompt is created with document input and JSON output fields matching the lab schema.
2. Testing with a PDF resume returns structured JSON without invented values for missing fields.
3. Testing with an image-based resume also returns valid JSON and records OCR confidence notes.
...
| Missing resume fields are hallucinated. | The instructions do not explicitly forbid guessing. | Strengthen the prompt to return empty strings for missing values. |
...
- At least two open job requisitions exist for testing.

### Success Criteria
1. The Job Requisitions table exists with the required columns and open records.
2. The matching prompt is configured to use Dataverse grounding and returns JSON with requisition matches.
3. The test output references real requisition numbers from Dataverse and the knowledge-used view shows grounded records being injected.

### Common Failure Modes
| Symptom | Root Cause | Resolution |
| --- | --- | --- |
| The Job Requisitions table is not selectable in the prompt builder. | The table changes were not published or the UI cache is stale. | Publish the table changes, refresh Copilot Studio, and retry the selector. |
| The prompt returns no matches or made-up requisition numbers. | The Dataverse filter is wrong or the prompt does not force grounded-only output. | Verify the filter value and add an explicit grounded-data-only instruction. |
| The prompt behaves like generic matching with no visible business data. | Too few fields or records are being retrieved from Dataverse. | Increase retrieval scope and include the requisition fields needed for matching. |

### Facilitator Escalation Trigger
Escalate if Dataverse grounding is unavailable or cannot surface live requisition data in the prompt experience.

## Lab 21 — Document Generation
### Pre-conditions
- Labs 13 and 20 are complete in the same environment.
- At least one candidate, application, and requisition record is available.
- Participant can edit Word templates and build agent flows.
- SharePoint or OneDrive storage is available for the template.

### Success Criteria
1. `OfferLetterTemplate.docx` exists with the required Plain Text Content Controls and the narrative prompt uses GPT-5 Chat or another GA fallback.
2. The `Generate Offer Letter` flow is published and follows the GA Word-template-plus-flow pattern rather than a non-GA path.
3. The `Create Offer Letter` topic returns a downloadable Word file whose fields and narrative are populated correctly.
...
3. The agent suggests interview-prep meeting options and can create the selected meeting, while the participant records one allowed action and one out-of-scope action.
...
### Pre-conditions
- Lab 13 is complete.
- User reactions are enabled in the Hiring Agent settings.
...
- The Hiring Agent is using a GA model baseline, with GPT-5 Chat preferred when available.
- Participant can access the Evaluation page and has working connections for any authenticated tools.

### Success Criteria
1. An evaluation run named `Hiring Agent - Day 2 QA Baseline` completes with at least five realistic test cases and a visible pass rate.
...
- The participant can open Hiring Agent in both VS Code and the browser.

### Success Criteria
1. The developer clones Hiring Agent into VS Code, opens `agent.yaml`, and sees no unresolved validation issues before editing.
