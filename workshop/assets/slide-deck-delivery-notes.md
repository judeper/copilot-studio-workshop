...
- **Live demo: Evaluation tab orientation.** Before Lab 24, open the **Evaluation** tab in Copilot Studio and walk the room through the three main areas: the **Test sets** list (where CSV-based test cases are uploaded), the **Graders** list (where assessment criteria are configured), and the **Run results** panel (where completed evaluation runs show pass/fail counts and individual case outcomes). Point out the "New evaluation run" button and explain the workflow: upload test set → select graders → run → review results. Participants who see this layout before Lab 24 will orient themselves immediately instead of searching for the right tab.
