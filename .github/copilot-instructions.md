...
- `workshop\assets\slide-deck-outline.md` is the canonical instructor presentation narrative and the text-first companion to the PPTX decks. It contains 92 numbered entries (85 baseline + 7 proposed concept slides marked `<!-- NEW -->` for Modules 04, 10, and 11) with a per-module slide count table showing both PPTX and outline counts. Use it for speaker notes, teaching intent, and structural review, and keep it aligned with the PPTX decks when slide sequencing, module boundaries, or teaching intent changes.
